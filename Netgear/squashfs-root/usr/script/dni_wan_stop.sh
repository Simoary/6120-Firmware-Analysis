...
if [ -f /var/run/dhcp6c_nas0.pid ] ; then
	#kill -9 `cat /var/run/dhcp6c_nas$i.pid`
	kill -SIGTERM `cat /var/run/dhcp6c_nas0.pid` 
	IS_TERM=0
	times=0
	...
	if [ "$times" = 20 ] ; then
		kill -9 `cat /var/run/dhcp6c_nas0.pid` 
	fi

	rm -f /var/run/dhcp6c_nas0.pid
fi

if [ -f /var/run/dhcp6c-nas0.info ] ; then
	rm -f /var/run/dhcp6c-nas0.info
fi
...
if [ -f /var/tmp/ipaddr6_nas0.msg ] ; then

	if [ -f /var/tmp/gw6_nas0.msg ] ; then 
		#/sbin/route -A inet6 del default gw `/var/tmp/gw6_nas$i.msg` dev nas$i 
		 ip route -A inet6 del default gw `/var/tmp/gw6_nas0.msg` dev nas0
	fi

# ifconfig control nas device, may influence smuxctl unregister nas device
# so use forground mode to run this command
	/sbin/ifconfig nas0 del `cat /var/tmp/ipaddr6_nas0.msg` 
	
	rm -f /var/tmp/ipaddr6_nas0.msg
	rm -f /var/tmp/gw6_nas0.msg
