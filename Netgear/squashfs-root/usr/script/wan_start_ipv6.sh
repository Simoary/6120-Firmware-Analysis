...
i=$1
k=$1
if [ "$i" = "1" ] && [ "$AYECOM_IPV6_6to4_TUNNEL_SUPPORT" != "" ] ; then
	/usr/script/dni_wan_start_ipv6.sh
else
		...
		if [ "$TCSUPPORT_UNIQUEMAC" != "" ] ;then
			if [ "$UNIQUEMAC_FLAG" = "1" ]; then
				...
				if [ "$WAN_MAC" != "" ]; then
					if [ "$DEFAULTROUTE" = "Yes" ] ; then
						if [ "$HOSTNAME" != "" ]; then
							/sbin/udhcpc -i nas$i -s /usr/script/udhcpc.sh -p /var/run/udhcpc-nas$i.pid -m $WAN_MAC -h $HOSTNAME &
						else
			...
			if [ "$TCSUPPORT_WPA_SUPPLICANT" = "" ] || [ "$IPOE_DOT1X_STATUS" != "Enable" ];then
				if [ "$WAN_MAC" != "" ]; then
					if [ "$DEFAULTROUTE" = "Yes" ] ; then
						if [ "$HOSTNAME" != "" ]; then
							/sbin/udhcpc -i nas$i -h $HOSTNAME -s /usr/script/udhcpc.sh -p /var/run/udhcpc-nas$i.pid -m $WAN_MAC &
						else
							/sbin/udhcpc -i nas$i -s /usr/script/udhcpc.sh -p /var/run/udhcpc-nas$i.pid -m $WAN_MAC &
						fi
					else	
						if [ "$HOSTNAME" != "" ]; then
							/sbin/udhcpc -i nas$i -h $HOSTNAME -s /usr/script/udhcpc_nodef.sh -p /var/run/udhcpc-nas$i.pid -m $WAN_MAC &
						else
							/sbin/udhcpc -i nas$i -s /usr/script/udhcpc_nodef.sh -p /var/run/udhcpc-nas$i.pid -m $WAN_MAC &
						fi
					fi
	...
	#		iptables -t filter -A FORWARD -i nas$i -j FORWARD_WAN
			iptables -t filter -I FORWARD 5 -i nas$i -j FORWARD_WAN
	fi
		fi
		fi
		WAN_IF=nas$i
	...
	#				iptables -t filter -A FORWARD -i ppp"$org_i""$serv_num" -j FORWARD_WAN
					iptables -t filter -I FORWARD 5 -i ppp"$org_i""$serv_num" -j FORWARD_WAN
				else	
					iptables -t nat -A PREROUTING -i ppp$i -j PREROUTING_WAN
	#				iptables -t filter -A FORWARD -i ppp$i -j FORWARD_WAN
					iptables -t filter -I FORWARD 5 -i ppp$i -j FORWARD_WAN
				fi	
			fi
		fi
		fi
