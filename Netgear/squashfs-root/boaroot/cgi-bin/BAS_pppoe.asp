<%
	if request_Form("submit_flag") <> "" then	
		...
		tcWebApi_set("Wan_PVC","SERVICE_NAME","pppoe_servername")
...
<form method="POST" action="/cgi-bin/BAS_pppoe.asp?id=<%tcWebApi_get("Timezone_Entry","GUITimestamp","s")%>" target=formframe>
...
var pppoe_get_mac_assign= "<%if tcWebApi_get("Wan_PVC","Mac_Assign","h") <> "N/A"  then tcWebApi_get("Wan_PVC","Mac_Assign","s") end if%>";
...
function loadvalue()
{
	...
	if(wan_type=="PPPoE LLC" || wan_type=="PPPoA LLC" || wan_type=="1483 Bridged IP LLC" || wan_type=="1483 Bridged Only LLC")
		form.ENCAP.value = "PPPoE LLC";
	else	
		form.ENCAP.value = "PPPoE VC-Mux";
	...
	<%end if%>
       if((wan_type.search("PPPoE") != -1 && active == "1") || form.WANAssign[1].checked == true){
	    if(pppoe_get_ip!="")
	    {
	...
	<%end if%>
        ...
        if((wan_type.search("PPPoE") != -1 && active == "1") || form.DNSAssign[1].checked == true){
	...
	aye_setMAC(pppoe_get_mac_assign);
...
<TR>
        <TD nowrap>
        <input type="radio" name="loginreq" id="loginreq" value="pppoe" checked="true"><%tcWebApi_multilingual("1","yes_str")%>
        </TD>
</TR>
<TR>
        <TD nowrap>
        <input onclick='location.href="BAS_ether.asp";' type="radio" name="loginreq" id="loginreq" value="dhcp"><%tcWebApi_multilingual("1","no_str")%>
        </TD>
</TR>
</TABLE>
</div>
...
<TR>
	...
	<TD nowrap align=right>
	<input class="pppoe_servername" type="text" name="pppoe_servername" id="pppoe_servername" maxLength="50" size="16" onKeyPress="return getkey('ssid', event)" value="<%If tcWebApi_get("Wan_PVC","SERVICE_NAME","h") <> "N/A" then tcWebApi_get("Wan_PVC","SERVICE_NAME","s") end if%>">
	</TD>
</TR>
...
<table id="MAC_Spoofing" border="0" cellspacing="3" width="100%">
<TR id=mac_head>
        <TD nowrap colspan=2>
...
<TR id=mac_default>
        <TD nowrap colspan=2>
        <input type="radio" name="MACAssign" id="mac_assign_type" value="0" onClick="aye_setMAC(0)"><%tcWebApi_multilingual("1","use_default_address_str")%>
        </TD>
</TR>
<TR id=mac_comp>
        <TD nowrap colspan=2>
