...
<body bgcolor=#ffffff>
<form method="POST" action="/cgi-bin/triggering.asp?id=<%tcWebApi_get("Timezone_Entry","GUITimestamp","s")%>">
<input type=hidden name=submit_flag value="triggering_add">

<div class="page_title"><%tcWebApi_multilingual("1","trigger_add_title_str")%></div>

<div class="fix_button">
<TABLE width=100% border=0 cellpadding=0 cellspacing=2>
<TR>
        <TD nowrap colspan=2 align=center>
			<input class="cancel_bt" type="reset" name="Cancel" id="cancel" value="<%tcWebApi_multilingual("0","glbstr_cancel")%>" onClick=location.href="triggering.asp";>&nbsp;&nbsp;
...
function chkPortRange()
{
		...
		{
			msg = str_invalid + str_pt_port + str_in_range;
			alert(msg);
			return true;
		}
		...
		{
			msg = str_invalid + str_in_port_start + str_in_range;
			alert(msg);
			return true;
		}
		
		if(port3 > 65535 || port3 < 1)
		{
			msg = str_invalid + str_in_port_end + str_in_range;
			alert(msg);
			return true;
		}
		...
		*/
		...
		if(checkPort(pt_port_str, cf.pt_port.value)==false)
			return true;
...
//              if(sertype=="UDP"&&serflag=="1")
    ...
    {
        consertype=table_Data[i][4];
        constart_port=table_Data[i][5];
        conend_port=table_Data[i][6];
...
function chkRuleLimit()
{
	...
	for(var i=0;i<document.forms[0].PortTriggering_CanUseNumFlag.value; i++)
	{
	...
	else
	{
		if(first_flag == 1)
		{
			document.forms[0].select_entry_num.value = input_rule;
		}
		else
			document.forms[0].select_entry_num.value = 0 ;
	}
...
function quotationCheck(object, limit_len) 
{
	...
	{
		if(object.value == table_Data[z][2])
		{
...
function doSave() 
{
	...
	document.forms[0].add_flag.value = 1;
	document.forms[0].PortTriggering_SubmitFlag.value = 1;
	document.forms[0].submit();
}
var pt_port_str="<%tcWebApi_multilingual("1","pt_port_str")%>";
...
<TR>
	<TD nowrap colspan=2><B><%tcWebApi_multilingual("1","inbound_con_str")%></B></TD>
</TR>
<TR>
	<TD nowrap><%tcWebApi_multilingual("1","connection_type_str")%></TD>
	<TD nowrap>
...
var endis_remote='<%if TCWebAPI_get("RemoteManager_Entry", "Activate", "h") <> "Yes" then asp_Write("0") else asp_Write("1") end if %>';'<%if TCWebAPI_get("RemoteManager_Entry", "Activate", "h") <> "Yes" then asp_Write("0") else asp_Write("1") end if %>';
</script>
</div>
