...
<input type=hidden name=submit_flag value="edit_qos_mac">
...
<TABLE width=100% border=0 cellpadding=0 cellspacing=2>
        <TR>
            ...
            <input class="apply_bt" type="submit" name="Apply" id="apply" value="Apply" onClick="return check_qos_mac(document.forms[0],'edit');">
            </TD>
        </TR>
</TABLE>
</div>
<div id="main" class="main_top_button"> 
<table id="client_mac_table" width="100%" border="0" cellpadding="0" cellspacing="3">
	...
	<TR>
		...
		<TD nowrap> 
			<SELECT name="category" id="category" onChange="change_priority()" disabled>
				...
				<OPTION value="3" selected>MAC Address</OPTION>			
			</SELECT>
		</TD>
	</TR>
		...
		if(flag == 0)
		{
			...
			//name[s]=attach_name_array[j];
                       ...
                       document.write("<TR class=\"odd_line\">");
                        document.write('<TD nowrap align=center><input type="radio" name="MacSelect" id="mac_select" value="'+i+'" disabled onclick="attached_qosmac_data_select('+i+','+j+')"></TD><TD nowrap align=center><span class="subhead">'+attach_policy[j]+'</span></TD>');
	...
	for(i=1;i<=qosmac_array_num;i++)
	{
		...
		if(each_qosinfo[0] == each_info[0])
		    if( i%2== 0 )
                       ...
                       document.write("<TR class=\"odd_line\">");
			document.write('<TD nowrap align=center><input type="radio" name="MacSelect" id="mac_select" checked value="'+i+'"  onclick="qosmac_data_select('+i+')"></TD>');
		else
			document.write('<TD nowrap align=center><input type="radio" name="MacSelect" id="mac_select" disabled value="'+i+'"  onclick="qosmac_data_select('+i+')"></TD>');		
		document.write('<TD nowrap align=center><span class="subhead">'+each_info[0].replace(/</g, "&#60;").replace(/>/g, "&#62;")+'</span></TD>');
		if(each_info[1]=='0')
...
<TR><TD colspan=2><img src="/liteblue.gif" width=100% height=12></TD></TR>
	<TR>
		...
		<TD nowrap><!-- 
			...
			<input type="text" value="" name="mac" id="mac" maxlength="60" size="16" onKeyPress="return getkeya(event)">
		</TD>
	</TR>
	<TR>
...
<TR><TD colspan=2><img src="/liteblue.gif" width=100% height=12></TD></TR>
	<TR align=center>
	    <TD nowrap colspan=2>
		<input class="add_bt"  type="submit" name="Add" id="add" value="Add" onClick="return valid_add_qosmac(document.forms[0],'add','edit_page');">
		<input class="edit_bt" type="submit" name="Edit" id="edit" value="Edit" onClick="return valid_add_qosmac(document.forms[0],'edit','edit_page');">
