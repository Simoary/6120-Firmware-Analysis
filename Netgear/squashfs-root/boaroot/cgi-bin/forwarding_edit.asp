...
<input type=hidden name=submit_flag value="forwarding_edit">
...
var lanip_addr="<%If tcWebApi_get("Lan_Entry","IP","h") <> "N/A" then tcWebApi_get("Lan_Entry","IP","s") end if%>";
...
function loadvalue()
{
	...
	addr_array = lanip_addr.split('.');
	cf.Prefix_IP.value = addr_array[0]+'.'+addr_array[1]+'.'+addr_array[2]+'.0/24';
	
	cf.select_entry_num.value = edit_parameter[0];
	cf.portname.value = edit_parameter[1];
	if(edit_parameter[2]== "UDP")
		cf.srvtype.value = "UDP";
	else if(edit_parameter[2] == "TCP")
		cf.srvtype.value = "TCP";
	else
		cf.srvtype.value = "TCP/UDP";
	cf.port_start.value = parseInt(edit_parameter[3]);
	cf.port_end.value = parseInt(edit_parameter[4]);
	
	cf.internal_port_start.value = parseInt(edit_parameter[5]);
	cf.internal_port_end.value = parseInt(edit_parameter[6]);
	if((cf.port_start.value==cf.internal_port_start.value)&& (cf.port_end.value==cf.internal_port_end.value))
	{
	...
	cf.service_ip.value = edit_parameter[7];
	var ip_array=edit_parameter[7].split('.');
...
function check_forwarding_add(cf)
{
  cf.internal_port_end.value = parseInt(cf.internal_port_start.value)+(parseInt(cf.port_end.value)-parseInt(cf.port_start.value));
	...
	{
			if(i==cf.select_entry_num.value)
			{
				continue;
			}
			if(table_Data[i][0]==cf.portname.value)
			{
	...
	{
		//skip selected rule port range check
		if(i==edit_parameter[0])
			continue;
...
*/
	cf.edit_flag.value = 1;
	cf.select_entry_num.value = cf.select_entry_num.value;
...
<TR>
	<TD nowrap>
	<input type="checkbox" name="same_range" id="same_range" value="same_range" onClick="portrange_value()">Use the same port range for Internal port
	</TD>
</TR>
<TR >
	<TD nowrap><%tcWebApi_multilingual("1","IS_port_str")%></TD>
	<TD nowrap>
	<input id="inter_start" type="text" name="internal_port_start"  size="7" maxlength="5" onKeyPress="return getkey('num', event)" onMouseOut="int_port_value()" onKeyUp="int_port_value()">(1~65534)
	</TD>
</TR>
<TR >
	<TD nowrap><%tcWebApi_multilingual("1","IE_port_str")%></TD>
	<TD nowrap>
	<input id="inter_end" type="text" name="internal_port_end" readonly size="7" maxlength="5" style="border:none" onKeyPress="return getkey('num', event)">
 	</TD>
</TR>
<TR>
...
<TR>
<TD></TD>
<TD nowrap><B><%tcWebApi_multilingual("1","sel_curr_device_str")%></TD>
</TR>
<TR>
<TD></TD>
<TD nowrap colspan=2>
	<TABLE id="attatch_device" border=1 cellpadding=2 cellspacing=0 width=100%>
	<TR>
...
var same_flag = 0;
	for(i=0;i<attach_array_len;i++)
	{
		document.write('<TR><TD nowrap align=center><input type="radio" name="MacSelect" id="mac_select" value="'+num+'" onclick="ipaddr_value()"></TD><TD nowrap align=center>'+attach_ip_array[i]+'</TD><TD nowrap align=center>'+attach_name_array[i]+'</TD></TR>');
		num = num +1;
	}
