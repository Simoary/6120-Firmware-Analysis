...
function check_remote()
{
	...
	else if(cf.rm_access[1].checked)
	{
		...
		{
			alert(invalid_remote_ip_range_str);
			return false;
		}
		...
		cf.start_ip.value=address_parseInt(start_ip)
		cf.end_ip.value=address_parseInt(end_ip);
		cf.remote_type.value =  1;
	}
...
*/
	if(cf.rm_access[2].checked)
		cf.remote_type.value =  2;
	
	if(cf.remote_mg_enable.checked == true)
	{
		cf.http_rmenable.value = "Yes";
		cf.rmwan_ip.value=address_parseInt(remote_manage_ip);
	}
	else
	{
		cf.http_rmenable.value = "No";
		cf.rmwan_ip.value=address_parseInt("0.0.0.0");
	}
	cf.remote_flag.value=1;
