...
function ayechange_radio(servicename,cf)
{
	var len = cf.service_type.options.length;
	
	for(i=0;i<len;i++)
	{
		if(servicename== serv_array[i][3])
		{
			cf.service_type.selectedIndex = i;
...
function check_block_services_add(cf,flag)
{
	...
	{
		alert(alert_invalid_starting_str);
		return false;
	}
	...
	for(i=0;i<txt.length;i++)
	{
		...
		{
			alert(alert_invalid_starting_port_str);
			return false;
		}
	}
	...
	for(i=0;i<txt.length;i++)
	{
		...
		if("0123456789".indexOf(c,0)<0)
		{
			alert(alert_invalid_ending_port_str);
			return false;
		}
	}
	...
	{
		alert(alert_ending_port_should_greater_str);
		return false;
	}
	...
	if(txt=="")
	{
		alert(alert_invalid_user_define_str);
		return false;
	}
	for(i=0;i<txt.length;i++)
	{
		...
		{
			alert(alert_invalid_user_define_servicetype_str);
			return false;
		}
	}
	...
	if(cf.iptype[0].checked == true)
	{
		...
		{
			alert(alert_this_ip_should_be_in_the_same_subnet_str);
			return false;
		}		
		cf.iplist.value=address_parseInt(cf.only_ip.value);
		cf.IPType.value="single";
	}
		...
		if(tableData[i][1]=="N/A")
			continue;
                ...
                if(flag == 'edit')
                {
                        ...
                        {
                                if((cf.service_type.value == tableData[i][2]||cf.userdefined.value == tableData[i][2].replace(/&nbsp;/g, " ")) && i!=cf.IndexID.value )
                                {
									
					alert(alert_the_blocked_service_already_exist_str);
					return false;
                        ...
                        {
                                if(cf.userdefined.value == tableData[i][2].replace(/&nbsp;/g, " ") && i!=cf.IndexID.value)
                                {
		...
		else
                {
                        if(cf.service_type.value != "User_Defined")
                        {
                                if((cf.service_type.value == tableData[i][2]||cf.userdefined.value == tableData[i][2].replace(/&nbsp;/g, " ")))
                                {
					alert(alert_the_blocked_service_already_exist_str);
					return false;
                        ...
                        {
                                if(cf.userdefined.value == tableData[i][2].replace(/&nbsp;/g, " "))
                                {
                                        alert(alert_the_blocked_service_already_exist_str);
                                        return false;
                                }
...
/*
	if(flag == 'edit'){
		//var str = eval ( 'custom_servicesArray' + select_edit );
		//service_info=str.split(' ');
		for(i=1;i<=tableData.length;i++){
			if(tableData[i][1]=="N/A")
				continue;
			//var item = eval ( 'block_servicesArray' + i );
			//block_info=item.split(' ');
			if(block_info[0] == service_info[2])
			{
				alert("$service_in_used")
				return false;
			}
		}
	}
*/
	...
	for(i=0;i<tableData.length;i++){
		//var str = eval ( 'custom_servicesArray' + i );
        //var each_info=str.split(' ');
		if(tableData[i][1]=="N/A")
			continue;
		...
		if(flag == 'edit')
		{
			if(cf.IndexID.value != i)
			{
				if(cf.userdefined.value == tableData[i][2])
				{
					  //location.href="dup_fail.htm";
                                          alert(alert_you_could_not_add_same_rule_str);
					  return false;
				}
				portinfo=tableData[i][3].split('-');
				if(cf.protocol.value == tableData[i][5] && cf.portstart.value == portinfo[0]  && cf.portend.value == portinfo[1])
				{
					//location.href="dup_fail.htm";
                                        alert(alert_you_could_not_add_same_rule_str);
					return false;
      ...
      if(cf.service_type.value == "User_Defined")
		  ...
		  for(i=0 ; i<serv_array.length ; i++)	
		{
				...
				portstart = serv_array[i][1];
				portend = serv_array[i][2];
				userdefined = serv_array[i][3];
				if(cf.userdefined.value == userdefined){
					//location.href="dup_fail.htm";
                                        alert(alert_you_could_not_add_same_rule_str);
					return false;
				}
				if(cf.protocol.value == protocol && cf.portstart.value == portstart && cf.portend.value == portend)
				{
					//location.href="dup_fail.htm";
                                        alert(alert_you_could_not_add_same_rule_str);
					return false;
...
function check_block_services_edit(cf)
{
	...
	if( array_num == 1)
	{
		if(cf.ruleSelect.checked == true)
		{
			count_select++;
			select_num=1;
			count_select_value = cf.ruleSelect.value;
		}
	}
		...
		if(cf.ruleSelect[i].checked == true)
		{
			count_select++;
			select_num=i+1;
			count_select_value = cf.ruleSelect[i].value;
		}
	if(count_select==0)
	{
		location.href="edit_fail.asp";
	...
	else
	{
		...
		location.href="BKS_service_edit.asp?index="+count_select_value;
		return true;
	}
}
...
function check_block_services_del(cf)
{
	...
	else
	{
		...
		//cf.action="/cgi-bin/BKS_service.asp";
		cf.submit();
		return true;
	}
}
