...
function check_schedule_onoff()
{
	...
	if(wireless_schedule_num > 1)
	{
		...
		{
			cf.priod_select[i].disabled = disable_flag;			
		}
	}
	else if(wireless_schedule_num >0)
	{
		cf.priod_select.disabled = disable_flag;
	}
	...
	if(wireless_schedule_num_a > 1)
	{
		...
		{
			cf.priod_select_an[i].disabled = disable_flag;				
		}
	}
	else if(wireless_schedule_num_a > 0)
		cf.priod_select_an.disabled = disable_flag;
}
...
//	if(cf.turnoff.checked && cf.enable_ap.checked && ntp_updated=="1")
...
function check_wireless_schedule(chan_type, btn_flag)
{
	...
	if(chan_type == "bgn")
	{
		if(btn_flag == "add")
		{
			...
			{
				alert("The router can support only 21 periods.");
				return false;
			}
			else
		...
		else if(btn_flag == "edit")
		{
			if(wireless_schedule_num > 0)
				...
				if(wireless_schedule_num > 1)
				{
                                	for(i=0; i<wireless_schedule_num; i++)
                                	{
                                        	if(cf.priod_select[i].checked)
                                                	cf.wladv_schedule_edit_num.value = cf.priod_select[i].value;
                                	}
				}
				else
				{
					if(cf.priod_select.checked)
						cf.wladv_schedule_edit_num.value = cf.priod_select.value;
				}	
...
//                                      alert("Please select an item to edit.");
 ...
 //                             alert("Please select an item to edit.");
				...
				if(wireless_schedule_num > 1)
				{
                                	...
                                	{
                                        	if(cf.priod_select[i].checked)
                                                	cf.wladv_schedule_delete_num.value = cf.priod_select[i].value;
                                	}
				}
				else
				{
					if(cf.priod_select.checked)
						cf.wladv_schedule_delete_num.value = cf.priod_select.value;
				}
		...
		else if(btn_flag == "edit")
		{
			if(wireless_schedule_num_a > 0)
			{
				...
				if(wireless_schedule_num_a > 1)
				{
					for(i=0; i<wireless_schedule_num_a; i++)
					{
						if(cf.priod_select_an[i].checked)
							cf.wladv_schedule_edit_num.value = cf.priod_select_an[i].value;
					}
				}
				else
				{
					if(cf.priod_select_an.checked)
						cf.wladv_schedule_edit_num.value = cf.priod_select_an.value;
				}
		...
		else if(btn_flag == "delete")
		{
			...
			{	
				...
				if(wireless_schedule_num_a > 1)
				{
					...
					{
						if(cf.priod_select_an[i].checked)
							cf.wladv_schedule_delete_num.value = cf.priod_select_an[i].value;
					}
				}
				else
				{
					if(cf.priod_select_an.checked)
						cf.wladv_schedule_delete_num.value = cf.priod_select_an.value;
				}
