...
function ipv6_write_ip(ipv6_ip_addr)
{
	...
	if(ipv6_ip_addr == "" || ipv6_ip_addr=="N/A")
	{
		document.write("<TR><TD nowrap><img src=/spacer.gif width=20 height=12 border=0>"+ipv6_address_not_avaible_str+"</TD></TR>");
	}
	else
	{
		...
		{
			document.write("<TR><TD nowrap><img src=/spacer.gif width=20 height=12 border=0>")
...
function ipv6_load_common(cf)
{
    ...
    if(ipv6_interface_type == "1")
    {
		...
		cf.IP_interface_1.value = interface_id_array[0];
		cf.IP_interface_2.value = interface_id_array[1];
		cf.IP_interface_3.value = interface_id_array[2];
		cf.IP_interface_4.value = interface_id_array[3];
...
function ipv6_save_common(cf)
{
    ...
    if( cf.enable_interface.checked == true )
    {
		...
		if( check_ipv6_IP_address(cf.IP_interface_1.value) == false )
		{
			alert(alert_invalid_interfaceid_str);
			return false;
		}
		if( cf.IP_interface_1.value == "" )
		{
			cf.IP_interface_1.value = "0";
		}
		
		if( check_ipv6_IP_address(cf.IP_interface_2.value) == false )
		{
			alert(alert_invalid_interfaceid_str);
			return false;
		}
		if( cf.IP_interface_2.value == "" )
		{
			cf.IP_interface_2.value = "0";
		}
		
		if( check_ipv6_IP_address(cf.IP_interface_3.value) == false )
		{
			alert(alert_invalid_interfaceid_str);
			return false;
		}
		if( cf.IP_interface_3.value == "" )
		{
			cf.IP_interface_3.value = "0";
		}
		
		if( check_ipv6_IP_address(cf.IP_interface_4.value) == false )
		{
			alert(alert_invalid_interfaceid_str);
			return false;
		}
		if( cf.IP_interface_4.value == "" )
		{
			cf.IP_interface_4.value = "0";
		}
		
		cf.ipv6_hidden_interface_id.value = cf.IP_interface_1.value + ":" + cf.IP_interface_2.value + ":" + cf.IP_interface_3.value + ":" + cf.IP_interface_4.value;
		...
		*/
		...
		if(ipv6_prefix!="" || ipv6_prefix!="N/A")
		{
			var tmp = ipv6_prefix.split(":");
			var tmp2 = ipv6_prefix.split("::");
			if(tmp.length==6)
				cf.LANIPV6.value = tmp2[0] +':'+ cf.ipv6_hidden_interface_id.value;
			else	
				cf.LANIPV6.value = tmp2[0] +'::'+ cf.ipv6_hidden_interface_id.value;
		}
    }
    else
    {
		...
		cf.LANIPV6.value="";
		cf.ipv6_hidden_interface_id.value="N/A";
    }
...
function set_interface()
{
	...
	if( cf.enable_interface.checked == true )
	{
			cf.IP_interface_1.disabled = false;
			cf.IP_interface_2.disabled = false;
			cf.IP_interface_3.disabled = false;
			cf.IP_interface_4.disabled = false;
	}
	else if( cf.enable_interface.checked == false )
	{	
			cf.IP_interface_1.disabled = true;
			cf.IP_interface_2.disabled = true;
			cf.IP_interface_3.disabled = true;
			cf.IP_interface_4.disabled = true;
	}
