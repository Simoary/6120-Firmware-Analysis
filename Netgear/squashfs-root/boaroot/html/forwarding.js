...
function check_list_length(forward_num)
{
	...
	{	
		if(forward_num==20)
		{
			alert(alert_the_router_can_support_20_str);
			return false;
		}
	}
	else
...
function check_port(cf)
{
	...
	{
		alert(alert_invalid_start_end_port_str);
		return false;
	}
	txt=parseInt(cf.port_start.value,10);
	if( isNaN(txt) )
	{
		alert(alert_invalid_start_port_str);
		return false;
	}
	...
	if(isNaN(txt))
	{
		alert(alert_invalid_end_port_str);
		return false;
	}
	...
	{
		alert(alert_invalid_end_port_1_str);
		return false;
	}
  ...
  cf.internal_port_end.value = parseInt(cf.internal_port_start.value)+(parseInt(cf.port_end.value)-parseInt(cf.port_start.value));
	...
	for(i=1;i<=forward_array_num;i++)
	{
		...
		if(flag == 'edit')
		{
			...
			{
				alert(alert_duplicate_service_name_str);
				return false;
			}
		}
		else
