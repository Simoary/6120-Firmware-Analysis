...
function change_pppoe_password(obj)
{
	if( obj.type == "password" )
	{
		...
		{
			obj.outerHTML = '<input class="pppoe_passwd" type="text" name="pppoe_passwd" maxlength="64" size="16" onFocus="this.select();" onKeyPress="return getkey(\'ssid\', event)" value="">';
...
/*
	...
	{
		alert(alert_password_cannot_blank_str);
		return false;
	}
*/
...
{
	if(check==1)
		form.runtest.value="true";
	else
		form.runtest.value="false";
	...
	{	
		...
		if(!parent.aye_regex_IPV4.exec(form.pppoe_ipaddr.value)){ alert(error_invalid_ip_address_str); return false; }

		
		
                if(checkipaddr(form.pppoe_ipaddr.value)==false || is_sub_or_broad(form.pppoe_ipaddr.value, lan_ip, lan_subnet) == false)
		{
			alert(alert_invalid_ipaddress_enter_again);
			return false;
		}
	...
	if (form.DNSAssign[1].checked == true)
	{
		...
		//	return false;
		if(!parent.aye_regex_IPV4.exec(form.pppoe_dnsaddr1.value)){ alert(error_invalid_primary_dns_str); return false; }
		if(form.pppoe_dnsaddr2.value!="")
			if(!parent.aye_regex_IPV4.exec(form.pppoe_dnsaddr2.value)){ alert(error_invalid_secondard_dns_str); return false; }	
	}
	...
	if(!parent.aye_regex_PPPUSERNAME.exec(form.pppoe_username.value)){ alert(error_invalid_username_str); return false; }
	if(!parent.aye_regex_PPPPASSWORD.exec(form.pppoe_passwd.value)){ alert(error_invalid_password_str); return false; }	
	
	if(form.pppoe_dod.value=="0")
		form.CONNECTION.value = "Connect_Keep_Alive";
	else if(form.pppoe_dod.value=="1")
		form.CONNECTION.value = "Connect_on_Demand";
	else if(form.pppoe_dod.value=="2")
		form.CONNECTION.value = "Connect_Manually";

    if(form.CONNECTION.value=="Connect_on_Demand")
    {
        if(!parent.aye_regex_PPPIDLETIME.exec(form.pppoe_idletime.value)){ alert(error_invalid_idletime_str); return false; }
    }
	
	//Use static IP
	if(form.WANAssign[1].checked == true)
		form.wan_PPPGetIP.value = "Static";
	else
		form.wan_PPPGetIP.value = "Dynamic";
		
	//NAT
	if(form.en_nat[0].checked)
		form.NATEnable.value = "Enable";
	else	
		form.NATEnable.value = "Disable";
		
	if(form.ENCAP.value=="PPPoA LLC" || form.ENCAP.value=="PPPoA VC-Mux")
		parent.menu_internet = "pppoa";
	else
		parent.menu_internet = "pppoe"; 
		
	//MAC Spoofing
	if(form.MACAssign[0].checked)
		form.WAN_MAC_ASSIGN.value="0";
	else if	(form.MACAssign[1].checked)
		form.WAN_MAC_ASSIGN.value="1";
	else
		form.WAN_MAC_ASSIGN.value="2";
	...
	form.Spoofmac.value = form.Spoofmac.value.toUpperCase();
