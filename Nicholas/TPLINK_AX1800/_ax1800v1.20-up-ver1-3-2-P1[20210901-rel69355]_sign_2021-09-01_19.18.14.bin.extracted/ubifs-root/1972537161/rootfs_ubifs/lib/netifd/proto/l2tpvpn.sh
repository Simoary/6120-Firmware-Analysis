...
ipsec_init_config() {
	...
	while true; do
		serverip=$(dnslookup $server -t 3)
		if [ ! $serverip]; then
			echo "server is $server,serverip= ?, wait 1s" > /dev/console
			sleep 1
			continue
			#return 1
		fi
...
#echo "	esp=3des-md5,3des-sha1,des-md5,des-sha1" >> ${ipsec_conf_temp}
	...
	: > ${ipsec_secrets_temp}
	echo "$localip $server : PSK \"$psk\"" >> ${ipsec_secrets_temp}
...
#	[ "$interval" != "$keepalive" ] || interval=5
	...
	if [ "$psk" != "" -a "$ipsec" = "1" ]; then
		...
		if [ $L2TP_VPN_SERVER_RUNNING != "yes" ]; then
			#wait 2 ipsec start up finished, but not guaranteed
			sleep 2
		fi
