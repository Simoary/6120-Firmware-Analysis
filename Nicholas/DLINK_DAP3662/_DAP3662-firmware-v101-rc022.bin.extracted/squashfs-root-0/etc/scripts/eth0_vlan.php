...
{    
	    ...
	    {	        
	        ...
	        if ($vlan_id_eth!="")			
	        { 
	            echo "brctl setpvidif br0 eth1 ".$vlan_id_eth."\n";
	        }
                if ($vlan_id_lan2!="")
                {
                    echo "brctl setpvidif br0 eth0 ".$vlan_id_lan2."\n";
                }
		...
		$ind=0;
		    ...
		    $lan2_egress= query($group_vid_path."/index:".$ind."/eth:2/egress"); 
		    $sys_egress=  query($group_vid_path."/index:".$ind."/sys:1/egress"); 
                    
		    if ($eth0_egress==1) 
			 {echo "brctl setgroupvidif br0 eth1 ".$group_vid." 1\n"; }   
                    else if ($eth0_egress==2)  {echo "brctl setgroupvidif br0 eth1 ".$group_vid." 0\n"; }

	            if ($lan2_egress==1)  
			 {echo "brctl setgroupvidif br0 eth0 ".$group_vid." 1\n"; }
		    else if ($lan2_egress==2)  {echo "brctl setgroupvidif br0 eth0 ".$group_vid." 0\n"; }
...
else
{
		...
		echo "brctl setpvidif br0 eth1   1 \n";
	        echo "brctl setsysvid br0 1\n";
            
            //    del all group vid
            echo "brctl delallgroupvidif br0 eth0  \n";
	    echo "brctl delallgroupvidif br0 eth1  \n";
