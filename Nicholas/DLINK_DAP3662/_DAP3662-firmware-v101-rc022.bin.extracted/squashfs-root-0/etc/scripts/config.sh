...
start)
	...
	echo "Inserting watchdog ..." > /dev/console
	insmod /lib/modules/external_wtd.ko
	echo enable > /proc/extwtd    
...
rgdb -i -s /runtime/wan/inf:2/mac "$prefix$emac"
nmac=`expr $mac + 9`
emac=`printf "%x" $nmac`
if [ $nmac -lt 16 ]
then
...
rgdb -i -s /runtime/wan/inf:2/macstart "$prefix$emac"
nmac=`expr $mac + 15`
emac=`printf "%x" $nmac`
if [ $nmac -lt 16 ]
then
...
rgdb -i -s /runtime/wan/inf:2/macend "$prefix$emac"
