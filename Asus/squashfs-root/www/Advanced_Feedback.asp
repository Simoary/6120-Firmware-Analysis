...
infolist.push(["<#1980#>","phone"]);
infolist.push(["QQ","qq"]);
infolist.push(["<#3717#>","wechat"]);
...
ptypelist.push(["<#1985#>", "Setting_Problem"]);
ptypelist.push(["<#1929#>", "Connection_or_Speed_Problem"]);
ptypelist.push(["<#1921#>", "Compatibility_Problem"]);
ptypelist.push(["<#1989#>", "Suggestion"]);
ptypelist.push(["<#2000#>", "Technical_Support"]);
...
desclist.push(["<#399#>/<#3457#>","USB dongle"]);
url_group.push(["Modem"]);
desclist.push(["<#1778#>","DM"]);
...
desclist.push(["<#2775#>","Network Tools"]); //25
url_group.push(["Status_"]);
desclist.push(["<#2989#>","Rescue"]);
url_group.push(["Rescue"]);//false value
desclist.push(["<#1925#>","Other Devices"]);
url_group.push(["Other_Device"]);//false value
desclist.push(["<#3716#>","Fail to access"]);
url_group.push(["GUI"]);//false value
desclist.push(["<#410#>","FW update"]);
url_group.push(["FirmwareUpgrade"]);
if(isSupport("Instant_Guard")){
desclist.push(["<#4189#>","Instant Guard"]); //30
url_group.push(["Instant_Guard"]);
}
}
else if(ptype == "Connection_or_Speed_Problem"){
desclist.push(["<#1930#>","Wireless speed"]);
desclist.push(["<#1931#>","Wired speed"]);
desclist.push(["<#1933#>","Unstable connection"]);
desclist.push(["<#1932#>","Router reboot"]);
desclist.push(["<#1934#>","Wireless disconnected"]);
}
else if(ptype == "Compatibility_Problem"){
desclist.push(["<#1924#>","modem"]);
desclist.push(["<#1926#>","other router"]);
desclist.push(["<#1922#>","OS or Application"]);
desclist.push(["<#1927#>","printer"]);
desclist.push(["<#1928#>","USB modem"]);
desclist.push(["<#1923#>","external hardware disk"]);
desclist.push(["<#1925#>","network devices"]);
}
else if(ptype == "Suggestion"){
desclist.splice(0,1);
desclist.push(["<#1992#>","Translation"]);
desclist.push(["<#1993#>","UI/UX"]);
desclist.push(["<#1990#>","Current Feature"]);
desclist.push(["<#1991#>","New Feature Request"]);
}
else if(ptype == "Technical_Support"){
desclist.splice(0,1);
desclist.push(["<#1998#>","tech_ASUS"]);
desclist.push(["<#1996#>","tech_Amazon"]);
desclist.push(["<#1999#>","tech_iOS"]);
desclist.push(["<#1997#>","tech_Android"]);
}
else{ //Other_Problem
desclist.splice(0,1);
...
timelist.push(["<#2012#>","Today"]);
timelist.push(["<#2013#>","Recently"]);
timelist.push(["<#2014#>","I don’t recall"]);
...
bandlist.push(["<#1389#>","All"]);
bandlist.push(["<#1969#>","I am not sure"]);
bandlist.push(["<#1970#>","Issue with the main router"]);
bandlist.push(["<#1971#>","Issue with node(s)"]);
...
unstablelist.push(["<#1967#>","All WiFi"]);
...
unstablelist.push(["<#1968#>","Both WiFi and WAN"]);
unstablelist.push(["<#1969#>","I am not sure"]);
unstablelist.push(["<#1970#>","Issue with the main router"]);
unstablelist.push(["<#1971#>","Issue with node(s)"]);
...
var remaining_time_str = "<#2740#> : ";
if(dblog_remaining == 0) {
remaining_time_str += "0" + " " + "(Prepare data...)"; //Untranslated
return remaining_time_str;
}
if(days)
remaining_time_str += days + " <#1683#> ";
if(hours)
remaining_time_str += hours + " <#2254#> ";
if(minutes)
remaining_time_str += minutes + " <#2631#> ";
if(seconds)
remaining_time_str += seconds + " <#3057#> ";
return remaining_time_str;
};
...
var dblog_service_mapping = ["", "WiFi", "<#1778#>", "<#776#>", "AiMesh"];
...
selectOption = {"1 <#2254#>" : hour_to_sec(1)};
}
if(usb_support && $("input[name=dblog_tousb_cb]").prop("checked")) {
baseOption = { "12 <#2254#>" : hour_to_sec(12), "1 <#1683#>" : hour_to_sec(24), "2 <#1683#>" : hour_to_sec(48), "3 <#1683#>" : hour_to_sec(72) };
}
else {
baseOption = { "6 <#2254#>" : hour_to_sec(6), "12 <#2254#>" : hour_to_sec(12), "24 <#2254#>" : hour_to_sec(24) };
}
...
$(".dblog_service_item.wifi").after(gen_appendix_option(16, "<#2005#>", "dhd"));
}
$("input[name=dblog_service_list]").prop("checked", true);
...
if(!nodm_support)
$(".dblog_service_item.all").after(gen_service_option(2, "<#1778#>", "noUSB"));
}
}
...
<div>&nbsp;</div>
<div class="formfonttitle"><#406#> - <#2620#></div>
<div style="margin:10px 0 10px 5px;" class="splitLine"></div>
<div id="fb_desc0" class="formfontdesc" style="display:none;"><#1950#></div>
<div id="fb_desc1" class="formfontdesc" style="display:none;"><#1951#></div>
<div id="fb_desc_disconnect" class="formfontdesc hint-color" style="display:none;"><#1952#> <a class="hint-color" href="mailto:router_feedback@asus.com?Subject=<%nvram_get("productid");%>" target="_top">router_feedback@asus.com</a></div>
...
<input type="checkbox" class="input" name="attach_cfgfile" id="attach_cfgfile_id"><label for="attach_cfgfile_id"><#1984#></label>&nbsp;&nbsp;&nbsp;
<span id="attach_iptables_span" style="color:#FFFFFF;"><input type="checkbox" class="input" name="attach_iptables" id="attach_iptables_id"><label for="attach_iptables_id"><#1964#></label></span>
<span id="attach_modem_span" style="color:#FFFFFF;"><input type="checkbox" class="input" name="attach_modemlog" id="attach_modemlog_id"><label for="attach_modemlog_id"><#1915#></label></span>
<input type="checkbox" class="input" name="attach_wlanlog" id="attach_wlanlog_id"><label for="attach_wlanlog_id"><#2004#></label>
</td>
</tr>
<tr>
<th><a class="hintstyle" href="javascript:void(0);" onClick="openHint(25,11);"><#1946#> *</a></th>
<td>
...
<br>
<span id="storage_ready" class="hint-color" style="display:none;">* <#3455#></span>
<span id="be_lack_storage" class="hint-color" style="display:none;">* <#2800#></span>
</td>
</tr>
...
<option value="3600">1 <#2254#></option>
<option value="18000">5 <#2254#></option>
<option value="43200">12 <#2254#></option>
<option value="86400">24 <#2254#></option>
<option value="172800">48 <#2254#></option>
</select>
</td>
</tr>
<tr class="dblog_support_class">
<th><a class="hintstyle" href="javascript:void(0);" onClick="openHint(34, 1);"><#1947#></a></th>
<td>
...
<label class="storeUSBHint hint-color"><input type="checkbox" name="dblog_tousb_cb" value="1" onclick="diag_change_storeUSB();" checked><#1948#></label>
<span class="noUSBHint hint-color">* <#2800#></span>
</div>
<div class="dblog_enabled_status">
<span>* <#1939#></span>
<br>
<span class="dblog_stop_text" onclick="dblog_stop();"><#1938#></span>
</div>
</td>
</tr>
...
<div class="dblog_disabled_status">
<label class="dblog_service_item all"><input type="checkbox" name="dblog_service_list_all" onclick="diag_change_service_list_all();"><#1389#></label>
</div>
<div class="dblog_enabled_status">
...
<option value="Stable_connection"><#1986#></option>
<option value="Occasional_interruptions"><#1978#></option>
<option value="Frequent_interruptions"><#1963#></option>
<option value="Unavailable"><#2001#></option>
</select>
</td>
</tr>
<tr>
...
<th><a class="hintstyle" href="javascript:void(0);" onClick=""><#2015#></a></th>
<td>
...
<th><a class="hintstyle" href="javascript:void(0);" onClick=""><#2007#></a></th>
<td>
...
<th><a class="hintstyle" href="javascript:void(0);" onClick=""><#1966#></a></th>
<td>
...
<th><a class="hintstyle" href="javascript:void(0);" onClick="openHint(34,2);"><#1414#></a></th>
<td>
<input type="text" name="fb_serviceno" maxlength="32" class="input_20_table" value="" autocorrect="off" autocapitalize="off">
</td>
</tr>
<tr style="display:none;">
<th><a class="hintstyle" href="javascript:void(0);" onClick="openHint(34,3);"><#1995#></a></th>
<td>
...
<span class="hint-color"><#1972#> : </span>
...
<div id="eula_content" style="margin-left: 20px;"><#2002#></div>
</div>
<input id="apply_button" class="button_gen" style="margin-left: 305px; margin-top:5px;" name="btn_send" onclick="applyRule()" type="button" value="<#1517#>"/>
<div id="loadingIcon" style="display:none;"><div class="loadingIcon" style="float: left; margin-left: 305px; margin-top:5px;"></div><div style="float: left; margin-left: 15px; margin-top:10px;"><span class="hint-color"><#369#>...</span></div></div>
</td>
</tr>
<tr>
<td colspan="2">
<strong><#2139#></strong>
<ul>
<li><#1977#><br><a id="call_link" style="font-weight: bolder;text-decoration:underline;cursor:pointer;" href="" target="_blank">https://www.asus.com/support/CallUs/</a></li>
</ul>
</td>
</tr>
</table>
