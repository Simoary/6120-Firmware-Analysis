...
$adv_igmp="IGMP侦听";
$adv_igmp_msg="Internet组管理协议(IGMP)侦听允许AP辨认在路由器和IGMP主机(无线STA)之间发送的IGMP请求和报告。"."当IGMP侦听启用时，如果有IGMP信息经过AP，那么AP会向IGMP主机转发组播数据包。";
if($check_band == "" && $switch != 1)
{
...
$adv_m2u="DHCP多播转单播";
$adv_m2u_msg="当设备多于30台时建议默认值为开启。由于设备不能从无线客户端模式获得IP地址，所以开启此功能时请选择静态IP。";
...
$adv_web_redirect = "网页重载";
$adv_web_redirect_msg = "网页将重定位到一个自定义的地址。点击选项以开启此功能。";
$adv_url_path = "网页地址";
$adv_url_path_msg = "输入一个IP地址或者域名。";
$adv_ticket = "密码";
...
$title_ipfilter = "IP过滤设置";
$title_ipfilter_msg = "用户不能连接到列表中的IP地址。";
$ipfilter_ip_address = "IP地址";
$ipfilter_ip_address_msg = "输入IP地址。";
$ipfilter_ip_mask = "子网掩码";
$ipfilter_ip_mask_msg = "输入子网掩码。";
$ipfilter_upload_download="上传和下载IP过滤文件";
$ipfilter_upload_download_msg="通过浏览保存的文件所在的位置更新IP过滤列表。单击打开，选择文件并单击上传。或者单击下载IP过滤文件下载文件到本地硬盘。每个ssid支持最大条目为64条。";
$title_macbypass = "MAC免认证";
$title_macbypass_msg = "列表中的MAC地址可以不通过强制网站的验证连接英特网。";
$macbypass_mac = "MAC地址";
$macbypass_mac_msg = "输入MAC地址。";
$macbypass_upload_download="上传和下载MAC免认证文件";
$macbypass_upload_download_msg="通过浏览保存的文件所在的位置更新MAC列表。单击打开，选择文件并单击上传。或者单击下载MAC免认证文件下载文件到本地硬盘。每个ssid支持最大条目为64条。";
...
{
$filters_acl_upload_download_msg="通过浏览保存的文件所在的位置更新MAC ACL列表。单击打开，选择文件并单击上传。或者单击下载ACL文件下载文件到本地硬盘。"."可以选择关闭/允许/拒绝以配置ACL文件规则。最大ACL条目为64条。";
}
else
{
...
$mt_upload_cwm="上传CWM配置文件";
$mt_upload_cwm_msg="浏览已保存的CWM配置文件目录，单击打开，然后上传文件以更新CWM配置。";
