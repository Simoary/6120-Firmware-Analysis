...
ffs_tc_add_up_rule()
{
    ...
    for i in $ifaces; do
        ...
        tc class add dev $wan_ifname parent 1:1 classid 1:1110 htb rate 82kbit ceil 82kbit 
        tc qdisc add dev $wan_ifname parent 1:1110 handle 1110: sfq perturb 10

        tc filter add dev $wan_ifname parent 1:0 protocol all handle 0x1110/0xffff fw classid 1:1110
    done
...
ffs_tc_del_up_rule()
{
    ...
    ffs_ratelimit_debug "ffs_del_qos_kctl_support start"
    ffs_del_qos_kctl_support
    ffs_ratelimit_debug "ffs_del_qos_kctl_support end"
}
...
ffs_fw_add_rate_rule()
{
    ...
    fw add 4 m limit_lan_br_wifi_LEVEL1 "MARK --set-xmark 0x1110/0xffff"
    fw add 4 m limit_lan_br_wifi_LEVEL1 "CONNMARK --set-xmark 0x1110/0xffff"
    ...
    fw add 4 m zone_wan_qos limit_wan_br_wifi_rule 1 { "-m connmark --mark 0x1110/0xffff" }
...
ffs_fw_del_rate_rule()
{
    ...
    # wan --> br-wifi 
    fw del 4 m zone_wan_qos limit_wan_br_wifi_rule { "-m connmark --mark 0x1110/0xffff" }
