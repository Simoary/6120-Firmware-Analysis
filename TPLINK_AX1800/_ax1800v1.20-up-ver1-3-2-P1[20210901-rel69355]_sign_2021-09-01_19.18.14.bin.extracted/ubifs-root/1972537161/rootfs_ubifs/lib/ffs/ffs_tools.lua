...
local function ffs_config_start(arg)
    local flag_boot_load_config = arg[2]
    local ffs_wireless_iface = uci_r:get("amazon_ffs", "ffs", "wireless_iface")
    local ffs_wireless_iface_ifname = uci_r:get("amazon_ffs", "ffs", "wireless_iface_ifname")
    ...
    if uci_r:get("network", "wifi") ~= "interface" then
        ...
        uci_r:set("network", "wifi", "ifname", ffs_wireless_iface_ifname)
        need_commit = true
    end
    ...
    if uci_r:get("wireless", ffs_wireless_iface) ~= "wifi-iface" then
        ...
        uci_r:set("wireless", ffs_wireless_iface, "ifname", ffs_wireless_iface_ifname)
        need_commit = true
    end
    
    --[[not need write flash]]
    if need_commit == true then
    	if flag_boot_load_config == "boot_load_config" then
...
local function ffs_network_change(arg)
    local ipaddr = arg[2]
    local submask = arg[3]
    local flag_boot_load_config = arg[4]
